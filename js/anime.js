...
InOut: [
    ...
    (t, f) = > t < .5 ? elastic(t * 2, f) / 2 : 1 - elastic(t * -2 + 2, f) / 2
/* InOutElastic */
]
}
...
function getInstanceTimings(type, animations, tweenSettings) {
    ...
    return arrayLength(animations) ? math.apply(Math, animations.map(anim = > anim[type])
) :
    tweenSettings[type];
}
...
function anime(params = {}) {
    ...
    function setAnimationsProgress(insTime) {
        ...
        while (i < arrayLength(animations)) {
            ...
            const tween = tweens.filter(t = > (insTime < t.end)
        )
...
function timeline(params) {
    ...
    tl.add = function (instancesParams) {
        tl.children.forEach(i = > {i.began = true;
        i.completed = true;
    })
        ;
